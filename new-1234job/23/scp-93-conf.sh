...
do
  ...
  #log_name2  
  sed -i "26c a1.sinks.k1.hdfs.fileSuffix = .log.$line" ${log_name}9.conf
  scp -P59522  ${log_name}9.conf  $line:/opt/elk/flume-1.7.0-dftthadoop2/conf/
  ssh -p59522 $line 'chown -R hadoop:hadoop /opt/elk/'
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/checkpoint/${log_name}9'"
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/data/${log_name}9'"
...
for line in `cat ip.txt`
do
  ssh -p59522 $line "ls /opt/elk/flume-1.7.0-dftthadoop2/conf/${log_name}9.conf"
