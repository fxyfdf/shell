...
do
  ...
  #log_name2  
  sed -i "26c a1.sinks.k1.hdfs.fileSuffix = .log.$line" ${log_name}2.conf
  scp -P59522  ${log_name}2.conf  $line:/opt/elk/flume-1.7.0-dftthadoop2/conf/
  ssh -p59522 $line 'chown -R hadoop:hadoop /opt/elk/'
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/checkpoint/${log_name}2'"
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/data/${log_name}2'"
  ssh -p59522 $line 'chown -R hadoop:hadoop /data/flume/checkpoint/'
  ssh -p59522 $line 'chown -R hadoop:hadoop /data/flume/data/'
  ...
  #log_name4
  sed -i "26c a1.sinks.k1.hdfs.fileSuffix = .log.$line" ${log_name}4.conf
  scp -P59522  ${log_name}4.conf  $line:/opt/elk/flume-1.7.0-tefam0/conf/
  ssh -p59522 $line 'chown -R hadoop:hadoop /opt/elk/'
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/checkpoint/${log_name}4'"
  ssh -p59522 $line "su - hadoop -c 'mkdir -p /data/flume/data/${log_name}4'"
  ssh -p59522 $line 'chown -R hadoop:hadoop /data/flume/checkpoint/'
  ssh -p59522 $line 'chown -R hadoop:hadoop /data/flume/data/'
...
do
  ...
  ssh -p59522 $line "ls /opt/elk/flume-1.7.0-dftthadoop2/conf/${log_name}2.conf"
  ssh -p59522 $line "ls /opt/elk/flume-1.7.0-ecgdbr/conf/${log_name}3.conf"
  ssh -p59522 $line "ls /opt/elk/flume-1.7.0-tefam0/conf/${log_name}4.conf"
