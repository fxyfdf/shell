...
do
i=$((i+1))
echo "----------$i------$line-------------" >> flume-start-all-s5qy3u.log
ssh -p52222 -tt   root@$line  >>flume-start-all-s5qy3u.log << EOF
...
sed -i '/root\|fi\|flume\|name\|done\|exit\|logout\|Last\|echo\|do/d' flume-start-all-s5qy3u.log
dos2unix flume-start-all-s5qy3u.log
exit
