...
do
i=$((i+1))
echo "----------$i------$line-------------" >> flume-start-all-5nxpdx-conf.log
for conf_name in `cat conf_name.txt`
do
ssh -p59522 -tt   root@$line  >>flume-start-all-5nxpdx-conf.log << EOF
...
sed -i '/root\|fi\|flume\|name\|done\|exit\|logout\|Last\|echo\|do/d' flume-start-all-5nxpdx-conf.log
dos2unix flume-start-all-5nxpdx-conf.log
exit
