...
do
i=$((i+1))
echo "----------$i------$line-------------" >> flume-start-all-1.7.0-conf.log
for conf_name in `cat conf_name.txt`
do
ssh -p59522 -tt   root@$line  >>flume-start-all-1.7.0-conf.log << EOF
...
sed -i '/root\|fi\|flume\|name\|done\|exit\|logout\|Last\|echo\|do/d' flume-start-all-1.7.0-conf.log
dos2unix flume-start-all-1.7.0-conf.log
exit
