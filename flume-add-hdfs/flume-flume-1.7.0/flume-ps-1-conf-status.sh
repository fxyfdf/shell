...
echo "----------$i------$line-------------" >> flume-ps-1-conf-status.log
for conf_name in `cat conf_name.txt`
do
ssh -p59522 -tt   root@$line  >> flume-ps-1-conf-status.log << EOF
...
sed -i '/root\|flume\|name\|done\|exit\|logout\|Last\|echo\|do/d' flume-ps-1-conf-status.log 
exit
